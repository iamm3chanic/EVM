...
int CVektor::Input(const char *fn, vector<CVektor*> &v, vector<CFabricVektor*> &fabric)
{
 ...
 //cout << "NON-PARALLEL FOR: TIME = "<<Time<<endl;
return 0;
}

int CVektor::ParallelTest(vector<CVektor*> &v)
{
 
 float Time=0, var; time_t t1,t2;
 //#pragma omp for
 //for(int val=0;getline(f,str);val++)
 time(&t1);
...
#pragma omp for //shared(w)
 ...
 localtime(&t2);
    Time=static_cast<double>(t2-t1);
 
 cout << "PARALLEL FOR: TIME = "<<Time<<endl;
return 0;
}
