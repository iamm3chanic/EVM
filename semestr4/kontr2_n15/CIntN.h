...
public:
    ...
    //void setN(int dimension) { n = dimension; pointer =(unsigned short int*)realloc(pointer, n*sizeof(unsigned short int));}   //setter
    ...
    /*unsigned short int operator *(const CIntN &v);*/
    unsigned short int &operator[](size_t i){if(i>pointer.size()-1) throw -1; return (this->pointer)[i];}
    //unsigned short int &operator[](int i){if(i<0||i>n-1) throw -1; return pointer[i];}
