...
public:
  CToken(){SetZero();}
  CToken(int num) {size=num; pos=0; s = new char[num]; memset(s,0, num*sizeof(char) );}
  CToken(const CToken&b){CopyOnly(b);}
  ~CToken(){Clean();}
  void SetN(int num) {Clean(); size=num; s = new char[num]; memset(s,0, num*sizeof(char) );}
  ...
  void SetZero(){s=NULL;size=0;pos=0;}
  void CopyOnly(const CToken&o){/*pos=o.pos;*/size=o.size;s=new char[size];memcpy(s,o.s,size);}
  void SetSize(int m){size=m;}
  void SetP(int p){pos=p;}
  void SetS(char* str){if(s!=NULL){delete[]s;} /* s = blocks[BLOCK_FLAG++]*/s= new char[size]; strncpy(s,str,size);}
