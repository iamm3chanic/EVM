...
int read_file(int fd)
{
    ...
    char *initial_buf_pos= new char[filesize]; // = buf;
    memcpy(initial_buf_pos, buf, filesize);
    ...
    if (noOfBlocks) //Just to check any single indirect pointers are used or not
    {
        ...
        memcpy(blockPointers, read_buf, BLOCK_SIZE); //sizeof(read_buf)
    ...
    if (noOfBlocks) //Indirect pointers done check for Double Indirect
    {
        ...
        block_read(block, read_buf);
        memcpy(indirectPointers, read_buf, BLOCK_SIZE); //sizeof(read_buf)
        int i = 0;
        while (noOfBlocks && i < 1024)
        {
    ...
    delete [] initial_buf_pos; delete [] buf; delete [] read_buf;
    return 1;
}
