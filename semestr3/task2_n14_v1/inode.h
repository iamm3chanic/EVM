...
// total disk size = 512KB
// total no of blocks = 512KB/4096 = 131072

#define DISK_BLOCKS 4096          /* number of blocks on the disk      */
#define BLOCK_SIZE 128            /* block size on "disk"              */
#define NO_OF_INODES 2048         /* In 50% possible number of inodes. */
...
public: 
   int filesize;
   int* pointer;
   Inode() {SetZero(); pointer = new int[12];}
   ~Inode() {Clean();}
   void SetZero() {filesize = 0; pointer = NULL;}
   void Clean() {delete [] pointer;}
   void CopyOnly(const Inode &b) {
       if(this!=&b)
	{Clean(); filesize=b.filesize;
	memcpy(pointer=new int[b.filesize],b.pointer, b.filesize*sizeof(int));} 
	}
   int* &GetPointer() {return pointer;}	
   int GetFilesize() {return filesize;}
   void SetPointer(int* p) {memcpy(pointer, p, 12);}
   void SetFilesize(int i) {filesize =i;}
};
/******************************************************************************/
...
int delete_disk(char *name);            /* delete a disk file                          */
