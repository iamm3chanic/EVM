...
int create_disk(char *disk_name)
{
    ...
    char* inode_buff = new char[len];
    memset(inode_buff, 0, len);
    memcpy(inode_buff, inode_arr, len);
    fwrite(inode_buff, sizeof(char), len, diskptr);
    delete [] inode_buff; delete [] dir_buff; delete [] sb_buff; delete [] buffer;
    fclose(diskptr);
    disks[disk_counter]=disk_name; disk_counter++;
    cout << string(GREEN) << "Virtual Disk Created!!!" << string(DEFAULT) << endl;

    return 1;
}

int delete_disk(char *name)
{
    memcpy(disk_name,name,50);
...
int mount_disk(char *name)
{
    ...
    char* sb_buff = new char[sizeof(sb)];
  ...
  inode_arr[NO_OF_INODES] */
    ...
    delete [] sb_buff; delete [] inode_buff; delete [] dir_buff;
...
int unmount_disk()
{
    ...
    delete [] sb_buff; delete [] dir_buff; delete [] inode_buff;
    active = 0;
    return 0;
}
...
int user_handle()
{
    ...
    while (1)
    {
        ...
        case 4:
            ...
            cin >> fd;
            cout << string(BOLD)<< "You're in write mode! Print :wq on separate line to exit." << string(DEFAULT)<< endl;
        ...
        case 5:
            ...
            cin >> fd;
            cout << string(BOLD)<< "You're in append mode! Print :wq on separate line to exit." << string(DEFAULT)<< endl;
...
int main()
{
    ...
    while (1)
    {
        cout << string(BOLD) << "Enter what to do:" << string(DEFAULT) << endl;
        cout << "1 : create disk" << endl;
        cout << "2 : mount disk" << endl;
        cout << "3 : delete disk" << endl;
        cout << "4 : list of disks" << endl;
        cout << "9 : exit" << endl;
        cin.clear();
        cin >> choice;
        if (choice == 9)
        ...
        case 3:
            cout << "Enter diskname : " << endl;
            cin >> disk_name;
            if (delete_disk(disk_name))
            {
              cout << string(GREEN) << "Disk deleted successfully! " << string(DEFAULT) << endl;
            }
            break;
        case 4:
        for(int i=0;access( disks[i], F_OK ) != -1;i++)
            {cout << "disk: " << disks[i] << endl;}
            break;
        default:
            cout << string(RED) << "Please make valid choice." << string(DEFAULT) << endl;
            cin.clear();
