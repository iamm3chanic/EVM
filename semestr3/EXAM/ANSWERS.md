...
«Время жизни» определяет, где переменная создается и где уничтожается. 
...
Динамический объект “живет” в поле свободной памяти, называемой кучей. Программист создает его с помощью оператора new, а уничтожает с помощью оператора delete. Динамически размещаться может как единичный объект, так и массив объектов. Размер массива, размещаемого в куче, разрешается задавать во время выполнения.

223.    С/С++. Область видимости переменных.

Область видимости связана со временм жизни переменных. Локальные видны внутри своих функций/блоков, глобальные - во всей программе. Добавить тут можно только о пространстве имен namespace. Переменные и функции из нэймспейса будут видны при его подключении.  
Подробнее: http://mycpp.ru/cpp/book/c08.html

224.    С/С++. Различие во времени рождения статических переменных.  

Статическая память выделяется еще до начала работы программы, на стадии компиляции и сборки. Статические переменные имеют фиксированный адрес, известный до запуска программы и не изменяющийся в процессе ее работы. Статические переменные создаются и инициализируются до входа в функцию main, с которой начинается выполнение программы. Статическая переменная со словом static от обычной глобальной отличается областью видимости: ststic видно только внути одного файла.  
1) Статическая переменная int остается в памяти во время работы программы. Нормальная или автоматическая переменная уничтожается, когда завершается вызов функции, в которой была объявлена переменная.  
2) Статическим переменным выделяется память в сегменте данных, а не в сегменте стека.  
3) Статические переменные (например, глобальные переменные) инициализируются как 0, если не инициализированы явно.

225.    С/С++. Для каких массивов существует существенное ограничение на размер?

Для массивов фиксированной длины, ведь память под них берется из стека, размер которого порядка 1Мб.  
Они создаются примерно так: int arr[500]

226.    C/C++. Что такое фактические и формальные параметры функций?

Формальные параметры объявляются в списке параметров в начале описания функции. Фактические - в описании. Например, определить функцию можно так:  
int sum(int, int);  
А описывать так:  
...
Интереснее всего тут объяснить на примере темплейтов. Когда в шаблон подставляется некоторый тип Т, код компилируется под этот отдельный тип. Очевидный пример, если мы подставим в функцию int sum(Т y, Т x) какое-то другое значение кроме int, компилятор "не сожрет" без cast'ов. Также можно сказать, что в любые функции при использовании подстаавляется заданный набор команд.

228.    С/С++. Что такое goto на уровне ассемблера?  

GOTO k  
Операнды:	0 <= k <= 1048575  
Операция:	k -> PC<20:1>  
Изменяет флаги:	Нет  
Описание:	Команда производит безусловный переход по любому адресу в пределах 2-х мегабайтного адресного пространства. 20-ти битное значение k загружается в PC<20:1> и выполняется за два цикла.  
http://www.microchip.ru/?cid=163&mid=3

229.    С/С++. Что такое вызов функции на уровне ассемблера?  

Процедуры идентифицируются по имени, после которого, собственно, и следует тело процедуры. Конец процедуры указывается стейтментом возврата.
...
Строка - это массив символов char. В конец строки добавляется \0.  
...
char *strdup(const char *str)|возвращает NULL – если не удалость выделить память, или Указатель на дублирующую строку 
char *strchr (const char *str, int ch)|возвращает Указатель на искомый символ, если он найден в строке str, иначе NULL
char *strstr(const char *strB, const char *strA)| возравщает NULL – если строка strA не входит в строку strB, или Указатель на первое вхождение строки strA в строку strB 
...
Перед использованием функции обязательно должен быть объявлен ее прототип. Обычно это делают и include-файле или в cpp-файле после всех инклюдов. Компилятор ищет в этом месте описание функции, и если не находит, то не компилирует.  
По поводу return, логично добавить, что функция должна возвращать значение, данное ей в определении, либо ничто, если она void. Иногда уместо написать пустой return.  
Хороший пример о прототипах тут: https://ravesli.com/urok-19-predvaritelnoe-obyavlenie-i-prototip-funktsii/  
...
Перед компиляцией есть возможность выбора во второй конструкции - можно исходя из условий использовать или не использовать какой-либо код из основной компиляции. Порядок работы такой: сначала процессор анализирует код, потом что-то включается или исключается из него, а потом происходит компиляция.  
if-else - обычный условный оператор, исполняется программой. 
 #if-#else-#endif - команды препроцессора, они выполняются ещё на этапе компиляции, но их можно вставить и в обычный код  
...
for row in range(len(A)):
	...
	for col in range(len(A[row])):
		print("  {1:5.2f}{0}".format(" " , A[row][col]), end='')
	print(")")
```
