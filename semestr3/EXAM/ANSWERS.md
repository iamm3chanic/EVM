...
Количество завершающий вершин = n!  
...
38.   Формулировка теоремы о нижней оценке времени решения задачи сортировки, основанной на сравнениях. (В этом вопросе важна точность формулировки)  
...
43.   Сколько и какой дополнительной памяти требует алгоритм слиянием с рекурсией? (Одинаково как с так и без рекурсии)  
...
44.   Сколько и какой дополнительной памяти требует алгоритм слиянием без рекурсии? (Одинаково как с так и без рекурсии)  
Алгортим сортировки слиянием тебует дополнительной памяти Θ(n) в куче(медленной памяти) для временных массивов и Θ(log2(n)) памяти в стеке (быстрой) для рекурсионного вызова функций ( log2(n) как максимальная глубина рекурсии)  
...
Среднее время работы (среднее арифметическое) алгоритма QSort на массиве из N различных элементов = O(n * log2(n))  
...
58.   Сколько дополнительной памяти требует цифровая сортировка? (*явного ответа в лекциях не нашел, из соображений логики*)  
...
64.   Теорема о существовании диаграммы Вороного. (*По построению?*)  
...
67.   Поиск порядковой статистики за линейное время в среднем.(*Алгоритм основан на QiuckSort*)
...
79.   C++. Понятия простых и сложных классов. (*не нашел строгое более определение*)
...
При вставке элемента баланс какого-либо элемента может нарушиться и стать равным +1 или -1 тогда необходимо с помощью 
...
218.    С/С++. Оценить сколько раз можно погрузиться в рекурсию в конструкции void f(){f();} (*Осторожно на сайте по-другому задан вопрос*)  
